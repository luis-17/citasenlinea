...
appRoot = angular.module('theme.core.main_controller', ['theme.core.services', 'blockUI'])
  .controller('MainController', ['$scope', '$route', '$uibModal', '$document', '$theme', '$timeout', 'progressLoader', 'wijetsService', '$routeParams', '$location','$controller'
    ,'rootServices', 'usuarioServices', 'blockUI', 'uiGridConstants', 'pinesNotifications','cronJobServices','controlEventoServices','progMedicoServices','grupoServices',
        ...
        if(response.flag == 1){
          ...
          if(idEmpresaMedico === null){
            ...
            if( $scope.fSessionCI.key_group === 'key_salud' || $scope.fSessionCI.key_group == 'key_dir_esp' 
                || $scope.fSessionCI.key_group == 'key_lab' || $scope.fSessionCI.key_group == 'key_salud_caja' || $scope.fSessionCI.key_group == 'key_salud_ocup' ) { 
              var arrData = {
                'iduser' : $scope.fSessionCI.idusers,
                'idmedico' : $scope.fSessionCI.idmedico,
              };
              rootServices.sListarEspecialidadesSession(arrData).then(function (rpta) {
